...
import { AdCartType, CategoryType } from "../../type";
import "./Hearder.css";

export default function Header() {
  const [category, setCategory] = useState<CategoryType[]>([]);
  const [search, setSearch] = useState("");
  const [searchResult, setSearchResult] = useState<CategoryType[] | AdCartType[]>([]);
  ...
  async function searchByAdOdCat(): Promise<void> {
    const result = await axios.get<CategoryType[] | AdCartType[]>(
          ...
          <div className="searchInput">
            ...
            {search && searchResult && (
              <ul className="dropDown">
                {searchResult.map((s) => (
                  <li key={s.id} className="dropDownItem">
                    {"title" in s ? <Link to={`/ads/${s.id}`}>{s.title} - Ad</Link>: <Link to={`/categoryList/${s.id}`}>{s.name} - cat</Link> }
                    </li>
                ))}
              </ul>
            )}
